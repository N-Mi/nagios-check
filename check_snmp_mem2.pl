...
my $nets_ram_buffer     = "1.3.6.1.4.1.2021.4.14.0"; # Real memory buffer
...
if (defined ($o_netsnmp)) {
  ...
  defined($o_cache) ? 
    ...
    :
    verb ("Ram : $$resultat{$nets_ram_free} ($$resultat{$nets_ram_cache} cached) ($$resultat{$nets_ram_buffer} buffer) / $$resultat{$nets_ram_total} : $realused");
  ...
  if (defined ($o_perf)) {
    if (defined ($o_cache)) {
      ...
      $n_output .= " | ram_used=" . ($$resultat{$nets_ram_total}-$$resultat{$nets_ram_free}-$$resultat{$nets_ram_cache}-$$resultat{$nets_ram_buffer}).";";
    }
